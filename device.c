...
#if defined(CONFIG_IR_GPIO_CIR)
#include <media/gpio-ir-recv.h>

static struct gpio_ir_recv_platform_data fs6818_gpio_ir = {
	.gpio_nr = PAD_GPIO_D + 8,
	//.gpio_nr = EXYNOS4_GPX0(5),
	//.gpio_nr = IRQ_EINT(5),
	.active_low = 1,
};

static struct platform_device fs6818_gpio_ir_device = {
	.name = "gpio-rc-recv",
	.dev = {
		.platform_data = &fs6818_gpio_ir,
	},
	.id = -1,
};
#endif
...
#include <linux/i2c.h>
#define	GT818_I2C_BUS		(1)

static struct i2c_board_info __initdata gt818_i2c_bdi = {
	.type	= "Goodix-TS",
	.addr	= (0x5d),
...
#if defined(CONFIG_FS6818B)
#define	WM8976_I2C_BUS		(2)
#else
...
#if defined(CONFIG_ZLG7290)
#include <linux/i2c.h>

#define	ZLG7290_I2C_BUS		(2)

/* CODEC */
static struct i2c_board_info __initdata zlg7290_i2c_bdi = {
	I2C_BOARD_INFO("zlg7290", 0x70>>1),

};
#endif

#if defined(CONFIG_LED1616) 
#include <linux/i2c.h>

#define	LED1616_I2C_BUS		(2)

/* CODEC */
static struct i2c_board_info __initdata led1616_i2c_bdi = {
	.type	= "led1616",		
	.addr	= (0x50),		
};
...
#if defined(CONFIG_ARDUINO_DC_MOTOR) 
#include <linux/i2c.h>

#define	DC_MOTOR_I2C_BUS		(2)

/* CODEC */
static struct i2c_board_info __initdata dc_motor_i2c_bdi = {
	.type	= "dc_motor",			
	.addr	= (0x4D),		
};
 ...
 */
#if defined(CONFIG_SENSORS_MMA8451) || defined(CONFIG_MXC_MMA8451_MODULE)
#include <linux/i2c.h>
...
#if defined(CONFIG_FS_ADC)
#ifndef CONFIG_ADC_SAMPLE_RATE
#define	CONFIG_ADC_SAMPLE_RATE		200000  /* 200K */
#endif
static struct resource fs6818_adc_resource[] = {
	[0] = DEFINE_RES_MEM(PHY_BASEADDR_ADC, SZ_1K),
	[1] = DEFINE_RES_IRQ(IRQ_PHY_ADC),
};
static struct platform_device fs6818_adc_device = {
	.name			= "fs6818-adc",
	.id				= -1,
	.num_resources	= ARRAY_SIZE(fs6818_adc_resource),
	.resource		= fs6818_adc_resource,
	.dev			= {
...
#if defined(CONFIG_FS_DHT11)
static struct resource xx18_dht11_resource[] = {
	[0] = DEFINE_RES_MEM(PHY_BASEADDR_GPIOD, SZ_1K),
	[1] = DEFINE_RES_MEM(PHY_BASEADDR_PWM, SZ_1K),
};
static struct platform_device xx18_dht11_device = {
	.name			= "dht11",
	.id				= -1,
	.num_resources	= ARRAY_SIZE(xx18_dht11_resource),
	.resource		= xx18_dht11_resource,
};
#endif

#if defined(CONFIG_FS_RFID)
#include <linux/i2c.h>
#define RFID_I2C_BUS		(2)
static struct i2c_board_info __initdata rfid_i2c_bdi[] = {
	{
		.type = "rfid",  //13.56Mhz module
		.addr = (0x19),
	},
	{
		.type = "rfid",   //nfc module
		.addr = (0x11),
	},
};
#endif
...
static int back_camera_power_enable(bool on)
{
    ...
    printk(KERN_ERR "%s: is_back_camera_enabled %d, on %d\n", __func__, is_back_camera_enabled, on);
...
#if 1
static void spi_set_level(unsigned line_id, int lvl)
{
	//printk(KERN_ERR "set_level = %d\n", lvl);
	nxp_soc_gpio_set_io_func(PAD_GPIO_B + 23, 1);
	nxp_soc_gpio_set_io_dir(PAD_GPIO_B + 23, 1);
	nxp_soc_gpio_set_out_value(PAD_GPIO_B + 23, lvl);
}
#endif

static struct s3c64xx_spi_csinfo spi0_info[] = {
	[0] = {
		.line = PAD_GPIO_B + 23,
		//.set_level = nxp_soc_gpio_set_out_value,
		.set_level = spi_set_level,
		.fb_delay = 0x2,
	},
}; 
...
{
    nxp_soc_gpio_set_io_func(PAD_GPIO_B + 27, 1);
	nxp_soc_gpio_set_io_dir(PAD_GPIO_B + 27, 1);

	if(enable) {
        nxp_soc_gpio_set_out_value(PAD_GPIO_B + 27, 1);
    	udelay(100);
        nxp_soc_gpio_set_out_value(PAD_GPIO_B + 27, 0);
    	udelay(100);
        nxp_soc_gpio_set_out_value(PAD_GPIO_B + 27, 1);
	}
	return 0;
...
static struct mcp251x_platform_data mcp2515_info = {
...
static struct platform_device back_light_device = {
	.name			= "fs6818-back-light",
};


static void back_light_power_enable(int on)
{
	nxp_soc_gpio_set_io_func(PAD_GPIO_B + 11, 2);
	nxp_soc_gpio_set_io_dir(PAD_GPIO_B + 11, 1);
	if (on)
    	nxp_soc_gpio_set_out_value(PAD_GPIO_B + 11, 1);
	else
    	nxp_soc_gpio_set_out_value(PAD_GPIO_B + 11, 0);
}
...
#if defined(CONFIG_ZLG7290) 
	printk("plat: add device zlg7290\n");
	i2c_register_board_info(ZLG7290_I2C_BUS, &zlg7290_i2c_bdi, 1);
#endif

#if defined(CONFIG_ARDUINO_DC_MOTOR) 
	printk("plat: add device dc_motor\n");
	i2c_register_board_info(DC_MOTOR_I2C_BUS, &dc_motor_i2c_bdi, 1);
#endif


	platform_device_register(&back_light_device);

#if defined(CONFIG_LED1616) 
	printk("plat: add device led1616\n");
	i2c_register_board_info(LED1616_I2C_BUS, &led1616_i2c_bdi, 1);
#endif
...
#if defined(CONFIG_TOUCHSCREEN_GT818)
	printk("plat: add touch(gt818) device\n");
	i2c_register_board_info(1, &gt818_i2c_bdi, 1);
#endif

#if defined(CONFIG_FS_RFID)
	printk("plat: add fs rfid device\n");
	i2c_register_board_info(RFID_I2C_BUS, &rfid_i2c_bdi, 2);
#endif

#if defined(CONFIG_FS_ADC)
	printk("plat: add a adc device\n");
	platform_device_register(&fs6818_adc_device);
#endif

#if defined(CONFIG_FS_DHT11)
	printk("plat: add a dht11 device\n");
	platform_device_register(&xx18_dht11_device);
#endif


#if defined(CONFIG_TOUCHSCREEN_GOODIX)
	printk("plat: add touch(gt9xx) device\n");
	i2c_register_board_info(GOODIX_I2C_BUS, &gt9xx_i2c_bdi, 1);
#endif
...
#elif defined(CONFIG_SENSORS_MMA8451) || defined(CONFIG_MXC_MMA8451_MODULE)
...
#if defined(CONFIG_IR_GPIO_CIR)
    printk("plat: add device gpio-rc-recv\n");
//	platform_device_register(&fs6818_gpio_ir_device);
#endif
	...
	printk("jinhc-----------------test here\n");
	camera_common_vin_setup_io(1, 1);
	back_camera_power_enable(1);
	back_light_power_enable(1);
